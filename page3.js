...
function startDnaHeart() {
  if (dnaHeartStarted) {
    return;
  }

  dnaHeartStarted = true;
...
function startGiftCountdown() {
  ...
  countdownTimer = window.setInterval(() => {
    ...
    if (countValue <= 0) {
      ...
      window.clearInterval(countdownTimer);
      window.setTimeout(showReveal, 500);
      return;
    }

    countNumber.textContent = String(countValue);
  }, 1000);
}
...
giftButton.addEventListener('click', startGiftCountdown);
giftClose.addEventListener('click', resetGiftOverlay);
